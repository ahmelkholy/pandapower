...
def convert_format(net, elements_to_deserialize=None):
    ...
    if (
        not isinstance(net.version, str)
        or not hasattr(net, "format_version")
        or Version(net.format_version) > Version(net.version)
    ):
...
def _add_missing_columns(net, elements_to_deserialize):
    ...
    if (
        ...
        and "const_z_percent" not in net.load
        or "const_i_percent" not in net.load
    ):
    ...
    if (
        ...
        and "tap_phase_shifter" not in net.trafo
        and "tp_phase_shifter" not in net.trafo
    ):
